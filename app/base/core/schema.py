...
from .utils.common import any_to_str, any_to_str_set, import_class
...
class Message(BaseModel):
        ...
        if ic and isinstance(ic, dict) and "class" in ic:
            if "mapping" in ic:
                ...
                actionnode_class = import_class(
                    "ActionNode", "base.core.action_node"
    ...
    def check_cause_by(cls, cause_by: Any) -> str:
        return any_to_str(
            cause_by
            if cause_by
            else import_class("UserRequirement", "base.core.add_requirement")
        )
    ...
    def ser_instruct_content(self, ic: BaseModel) -> Union[dict, None]:
        ...
        if ic:
            ...
            ic_type = str(type(ic))
            if "<class 'base.core.action_node" in ic_type:
    ...
    def __init__(self, content: str = "", label="", **data: Any):
        data["content"] = data.get("content", content)
        data["label"] = data.get("label", label)  # NOTE: Place it as None if not provided
        super().__init__(**data)
...
class Task(BaseModel):
    task_id: str = ""
    dependent_task_ids: list[str] = Field(default_factory=list)  # Tasks prerequisite to this Task
    instruction: str = ""
    task_type: str = ""
    code: str = ""
    result: str = ""
...
class CodePlanAndChangeContext(BaseModel):
    ...
    def loads(filenames: List, **kwargs) -> CodePlanAndChangeContext:
        ctx = CodePlanAndChangeContext(
            requirement=kwargs.get("requirement", ""), issue=kwargs.get("issue", "")
        )
        for filename in filenames:
