    ...
    async def _act(self) -> Message:
        ...
        if self.rc.todo.name=="GeneralVerify":
            try:
                ...
                repair_reports=OverallRepair(self.rc.memory.get_by_label("repair_report")[-1].content)
                ...
                verify_report_msg=Message(content=res_plain,label="verify_report",role=self.profile,cause_by=type(todo),sent_from=type(todo))
