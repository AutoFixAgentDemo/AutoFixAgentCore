...
def decode_unicode_escape(s, pos):
    """
    Decodes a Unicode escape sequence starting at position `pos` in the string `s`. Replace the _decode_uXXXX imported from json.decoder.

    Args:
        s (str): The input string.
        pos (int): The position where the Unicode escape sequence starts.

    Returns:
        tuple: A tuple containing the decoded character and the new position.
    ...
    if len(esc) == 4:
        try:
            # Decode the hexadecimal Unicode code point
            char = chr(int(esc, 16))
            return char, pos + 5
        except ValueError:
            pass
    raise ValueError(f"Invalid \\uXXXX escape at position {pos}")
...
def py_scanstring(s, end, strict=True, _b=BACKSLASH, _m=STRINGCHUNK.match, delimiter='"'):
    ...
    while 1:
        ...
        if esc != "u":
            ...
            uni = decode_unicode_escape(s, end)
            end += 5
            if 0xD800 <= uni <= 0xDBFF and s[end: end + 2] == "\\u":
                uni2 = decode_unicode_escape(s, end + 1)
